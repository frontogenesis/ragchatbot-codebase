...
class TestFailureScenarios:
    ...
    def test_chroma_db_corruption_failure(self, test_config):
        ...
        with (
            ...
            mock_vector_store_instance.search.return_value = SearchResults.empty(
                "Search error: Database file is corrupted"
            )
    ...
    def test_embedding_model_download_failure(self, test_config):
        ...
        with (
            ...
            mock_vector_store.side_effect = Exception(
                "Failed to download model 'all-MiniLM-L6-v2': Network error"
            )
    ...
    def test_out_of_disk_space_failure(self, test_config):
        ...
        with (
            ...
            mock_vector_store_instance.search.return_value = SearchResults.empty(
                "Search error: No space left on device"
            )
            mock_ai_gen.return_value.generate_response.return_value = (
                "I'm unable to search due to storage limitations."
            )
            ...
            assert (
                "unable to search" in response.lower() or "storage" in response.lower()
            )
    ...
    def test_network_connectivity_failure(self, test_config):
        ...
        with (
            ...
            assert "Network error" in str(exc_info.value) or "Connection" in str(
                exc_info.value
            )
    ...
    def test_concurrent_access_race_condition(self, test_config):
        ...
        with (
            ...
            rag_system.tool_manager.get_last_sources = Mock(
                side_effect=[["Course A"], [], ["Course B"], []]
            )
            rag_system.tool_manager.get_last_source_links = Mock(
                side_effect=[["link1"], [], ["link2"], []]
            )
            ...
            error_responses = [
                r for r in responses if "error" in r.lower() or "busy" in r.lower()
            ]

            assert (
                len(success_responses) > 0
            ), "No successful responses during race condition test"
            assert (
                len(error_responses) > 0
            ), "No error responses during race condition test"
    ...
    def test_document_processing_failure_empty_system(self, test_config):
        ...
        with (
            ...
            mock_ai_gen.return_value.generate_response.return_value = (
                "I don't have any course materials loaded to search through."
            )
    ...
    def test_session_management_corruption(self, test_config):
        ...
        with (
            ...
            mock_session.return_value.get_conversation_history.side_effect = Exception(
                "Session data corrupted"
            )
            mock_session.return_value.add_exchange.side_effect = Exception(
                "Cannot save session"
            )

            mock_ai_gen.return_value.generate_response.return_value = (
                "I cannot access conversation history due to session issues."
            )
            ...
            response, sources, source_links = rag_system.query(
                "Continue our conversation", session_id="corrupted-session"
            )
    ...
    def test_tool_definition_corruption_failure(self, test_config):
        ...
        with (
            ...
            rag_system.tool_manager.get_tool_definitions = Mock(
                return_value=[
                    {
                        "name": "corrupted_tool",
                        "invalid_schema": True,
            ...
            mock_ai_gen.return_value.generate_response.return_value = (
                "I encountered an issue with the search tools."
            )
    ...
    def test_cascading_failure_scenario(self, broken_config):
        ...
        with (
            ...
            response, sources, source_links = rag_system.query(
                "Can you explain what machine learning is according to the course materials?"
            )
    ...
    def test_system_recovery_after_failure(self, test_config):
        ...
        with (
            ...
            search_results_sequence = [
                SearchResults.empty(
                    "Search error: Temporary failure"
                ),  # Initial failure
            ...
            rag_system = RAGSystem(test_config)
            rag_system.tool_manager.get_last_sources = Mock(
                side_effect=[[], ["AI Fundamentals - Lesson 1"]]
            )
            rag_system.tool_manager.get_last_source_links = Mock(
                side_effect=[[], ["https://ai.com"]]
            )
    ...
    def test_partial_failure_scenarios(self, test_config):
        ...
        with (
            ...
            mock_ai_gen.return_value.generate_response.return_value = (
                "Here's what I found about neural networks in the course."
            )
            ...
            rag_system.tool_manager.get_last_sources = Mock(
                return_value=["Deep Learning - Lesson 3"]
            )
            rag_system.tool_manager.get_last_source_links = Mock(
                return_value=[None]
