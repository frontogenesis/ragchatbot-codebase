...
class TestCourseSearchToolComprehensive:
    ...
    def test_execute_with_course_name_resolution_failure(self):
        ...
        course_not_found_error = SearchResults.empty("No course found matching 'NonexistentCourse'")
    ...
    def test_execute_with_realistic_vector_store_configuration(self):
        ...
        for config in test_configs:
            ...
            if config["expected_result_count"] == 0:
                ...
                documents = [f"Document content {i}" for i in range(config["expected_result_count"])]
                metadata = [
                    {"course_title": "Test Course", "lesson_number": i + 1, "chunk_index": i}
                    for i in range(config["expected_result_count"])
                ]
                distances = [0.1 * (i + 1) for i in range(config["expected_result_count"])]
                search_results = SearchResults(documents=documents, metadata=metadata, distances=distances)
    ...
    def test_execute_with_network_timeout_simulation(self):
        ...
        timeout_error = SearchResults.empty("Search error: Request timeout after 30 seconds")
    ...
    def test_execute_with_large_result_set_handling(self):
        ...
        documents = [f"Large document content {i} " * 100 for i in range(large_document_count)]  # Long documents
        metadata = [
            {"course_title": f"Course {i % 10}", "lesson_number": i % 20, "chunk_index": i}
            for i in range(large_document_count)
        ]
        distances = [0.001 * i for i in range(large_document_count)]
        
        large_results = SearchResults(documents=documents, metadata=metadata, distances=distances)
    ...
    def test_execute_parameter_validation_edge_cases(self):
        ...
        edge_case_parameters = [
            ...
            {"query": "test", "course_name": "Course with / and \\ and ?", "lesson_number": None},
        ]
    ...
    def test_execute_memory_cleanup_after_large_operations(self):
        ...
        for iteration in range(10):
            large_results = SearchResults(
                ...
                metadata=[{"course_title": f"Course {iteration}", "lesson_number": i} for i in range(50)],
                distances=[0.01 * i for i in range(50)]
            )
            mock_vector_store.search.return_value = large_results
            mock_vector_store.get_lesson_link.return_value = f"https://example.com/lesson{iteration}"
