...
class TestRAGSystemIntegration:
    ...
    def test_query_end_to_end_with_proper_config_success(self, test_config):
        ...
        with (
            ...
            mock_ai_gen.return_value.generate_response.return_value = (
                "Based on the course materials, lesson 1 covers introduction to AI."
            )
            mock_session.return_value.get_conversation_history.return_value = None
            ...
            rag_system.tool_manager.get_last_sources = Mock(
                return_value=["AI Fundamentals - Lesson 1"]
            )
    ...
    def test_query_with_empty_vector_store(self, test_config):
        ...
        with (
            ...
            mock_ai_gen.return_value.generate_response.return_value = (
                "I don't have information about that topic in the course materials."
            )
            ...
            assert (
                "don't have information" in response.lower() or "no" in response.lower()
            )
            assert sources == []
            assert source_links == []
    ...
    def test_query_with_corrupted_vector_store_data(self, test_config):
        ...
        with (
            ...
            mock_ai_gen.return_value.generate_response.return_value = (
                "I found some information but it appears to be corrupted."
            )
    ...
    def test_query_with_network_timeout_simulation(self, test_config):
        ...
        with (
            ...
            mock_vector_store_instance.search.return_value = SearchResults.empty(
                "Search error: Request timeout"
            )

            mock_ai_gen.return_value.generate_response.return_value = (
                "The search operation timed out. Please try again later."
            )
    ...
    def test_query_flow_with_session_management_errors(self, test_config):
        ...
        with (
            ...
            mock_ai_gen.return_value.generate_response.return_value = (
                "Response without session context"
            )
            ...
            response, sources, source_links = rag_system.query(
                "What is AI?", session_id="broken-session"
            )
    ...
    def test_query_with_tool_registration_failure(self, test_config):
        ...
        with (
            ...
            mock_ai_gen.return_value.generate_response.return_value = (
                "I don't have access to course search tools."
            )
            ...
            assert (
                "don't have access" in response.lower() or "tools" in response.lower()
            )
    ...
    def test_query_with_document_processing_never_completed(self, test_config):
        ...
        with (
            ...
            mock_ai_gen.return_value.generate_response.return_value = (
                "No course materials are currently loaded in the system."
            )
    ...
    def test_query_source_tracking_integration(self, test_config):
        ...
        with (
            ...
            response, sources, source_links = rag_system.query(
                "Compare lesson content across courses"
            )
            ...
            with (
                ...
                if scenario["layer"] == "vector_store":
                    mock_vector_store.return_value.search.return_value = (
                        SearchResults.empty(f"Search error: {scenario['error']}")
                    )
                    mock_ai_gen.return_value.generate_response.return_value = (
                        f"I encountered an error: {scenario['error']}"
                    )
    ...
    def test_query_performance_with_realistic_load(self, test_config):
        ...
        with (
            ...
            mock_vector_store_instance.search.return_value = SearchResults(
                ...
                metadata=[
                    {"course_title": f"Course {i}", "lesson_number": 1}
                    for i in range(5)
                ],
            ...
            mock_ai_gen.return_value.generate_response.return_value = (
                "Response based on multiple course sources"
            )

            rag_system = RAGSystem(test_config)
            rag_system.tool_manager.get_last_sources = Mock(
                return_value=[f"Course {i} - Lesson 1" for i in range(5)]
            )
            rag_system.tool_manager.get_last_source_links = Mock(
                return_value=[f"https://course{i}.com" for i in range(5)]
            )

            # Execute multiple queries in sequence
            for i in range(10):
                response, sources, source_links = rag_system.query(
                    f"Query {i} about course content"
                )
