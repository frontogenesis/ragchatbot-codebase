...
class TestSystemDiagnosis:
    ...
    def test_config_validation_max_results_zero_bug(self):
        ...
        if current_config.MAX_RESULTS == 0:
            pytest.fail(f"CRITICAL BUG FOUND: MAX_RESULTS is set to {current_config.MAX_RESULTS}. This will cause all vector searches to return empty results!")
        
        assert current_config.MAX_RESULTS > 0, f"MAX_RESULTS must be > 0, got {current_config.MAX_RESULTS}"
        
        # Test recommended values
        assert current_config.MAX_RESULTS >= 3, "MAX_RESULTS should be at least 3 for useful results"
        assert current_config.MAX_RESULTS <= 20, "MAX_RESULTS should not exceed 20 to avoid performance issues"
    ...
    def test_config_anthropic_api_key_validation(self):
        ...
        if not api_key or api_key == "":
            pytest.skip("ANTHROPIC_API_KEY not set - this will cause query failures. Set ANTHROPIC_API_KEY environment variable.")
        ...
        placeholder_values = ["your_api_key_here", "test-api-key", "placeholder", "change_me"]
    ...
    def test_config_model_name_validation(self):
        ...
        assert current_config.ANTHROPIC_MODEL == expected_model, f"Model should be {expected_model}, got {current_config.ANTHROPIC_MODEL}"
    ...
    def test_config_chunk_size_settings(self):
        ...
        assert 200 <= current_config.CHUNK_SIZE <= 2000, f"CHUNK_SIZE {current_config.CHUNK_SIZE} may be too small or too large"
        
        # Test chunk overlap
        assert current_config.CHUNK_OVERLAP >= 0, "CHUNK_OVERLAP must be non-negative"
        assert current_config.CHUNK_OVERLAP < current_config.CHUNK_SIZE, "CHUNK_OVERLAP must be less than CHUNK_SIZE"
        
        # Test reasonable overlap percentage
        overlap_percentage = (current_config.CHUNK_OVERLAP / current_config.CHUNK_SIZE) * 100
        assert overlap_percentage <= 50, f"Chunk overlap {overlap_percentage:.1f}% seems excessive"
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                if test_config.MAX_RESULTS == 0:
                    pytest.fail("Vector store initialized with MAX_RESULTS=0 - this will cause search failures!")
    ...
    def test_ai_generator_initialization_with_current_config(self):
        ...
        try:
            ai_generator = AIGenerator(
                current_config.ANTHROPIC_API_KEY,
                current_config.ANTHROPIC_MODEL
            )
    ...
    def test_rag_system_initialization_with_current_config(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            test_config = Config()
            test_config.CHROMA_PATH = temp_dir
            test_config.ANTHROPIC_API_KEY = current_config.ANTHROPIC_API_KEY or "test-key"
            test_config.MAX_RESULTS = current_config.MAX_RESULTS
            
            if test_config.MAX_RESULTS == 0:
                pytest.fail("Cannot test RAG system with MAX_RESULTS=0 - this is a critical bug!")
    ...
    def test_docs_folder_existence_and_content(self):
        ...
        try:
            ...
            course_files = [f for f in files if f.lower().endswith(('.pdf', '.docx', '.txt'))]
            
            if not course_files:
                pytest.skip(f"No course documents found in {docs_path} - system will have no content")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                test_course = Course(
                    ...
                    lessons=[Lesson(lesson_number=1, title="Test Lesson", lesson_link="https://test.com/1")]
                )
                ...
                if current_config.MAX_RESULTS > 0 and results.is_empty():
                    pytest.fail("Search returned empty results despite having data - possible configuration issue")
    ...
    def test_system_memory_requirements(self):
        ...
        if available_memory_gb < 2:
            print(f"Warning: Low available memory ({available_memory_gb:.1f} GB) - may cause performance issues")
        ...
        if os.path.exists(chroma_path):
            ...
            if available_space_gb < 1:
                print(f"Warning: Low disk space ({available_space_gb:.1f} GB) in {chroma_path}")
    ...
    def test_current_working_directory_context(self):
        ...
        if not any(path in current_dir for path in expected_paths):
            ...
            print("Consider running tests from the backend directory: cd backend && python -m pytest")
    ...
    def test_logging_configuration(self):
        ...
        print(f"Current logging level: {current_level} ({logging.getLevelName(current_level)})")
    ...
    def test_system_diagnosis_summary(self):
        ...
        print(f"API Key configured: {'Yes' if current_config.ANTHROPIC_API_KEY else 'No'}")
